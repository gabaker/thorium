...
- **api** (`thorium-api`) — The core REST API server. Uses Axum. Exposes the `thorium` library crate containing the API server, client, and data models.
  ...
  - `api/src/models/` — Data models with backend implementations (`models/backends/` for DB layer). **Frontend mirrors:** `ui/src/models/` contains equivalent TypeScript types. When modifying API resource models, you MUST update the corresponding TypeScript types in `ui/src/models/` to stay in sync.
...
ui/
  src/
    ...
    fonts/
    models/             # TypeScript type definitions — must stay in sync with `api/src/models/`
    pages/              # route-level page components
      GraphBuilder.tsx  # graph page (/graph) — renders seed input + graph + tree
      Home.tsx, Login.tsx, NotFound.tsx
      entities/         # config-driven entity CRUD pages (files, repos, collections, devices, vendors, file_systems, etc.)
      images/           # image browsing, create
      pipelines/        # pipeline browsing, create
...
- `tsconfig.json` — extends base, includes `checkJs: true` (stricter check; used by `npm run build-check`)
