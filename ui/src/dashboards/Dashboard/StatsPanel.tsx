 ...
 * the default; `FileTypeExtension` (the filename extension) is the alternate.
 */
const TAG_KEY_OPTIONS = ['FileType', 'FileTypeExtension'] as const;
 ...
 * memoized on `[graphVersion, visibleSet]`.
 *
 ...
 *   `FileType` with `FileTypeExtension` also offered via a small key picker (component state). Clicking a
 *   bar toggles the value into the key's single is-one-of tag clause via {@link toggleTagValue}.
 *
...
const StatsPanel: React.FC<StatsPanelProps> = ({ clauses, setClauses, onBarClick, seed }) => {
  ...
  // node set = only the visible ids when a filter is active, otherwise every node in the graph (same
  // downselect as the tags tile, via the shared visibleNodes helper). graphVersion bumps whenever
  // data_map changes; graph is a stable ref
  const nodes = useMemo(() => visibleNodes(graph.data_map, visibleSet), [graph, graphVersion, visibleSet]);
  ...
  // makeIncludeClause so the Include-clause shape stays identical to deriveStats' barToClause
