...
import { AddButton, BrowseControls, BrowseRow, BuilderSection, EmptyBrowse, TagModeRow } from './styles';
...
const FileUploadsFirst: React.FC<{
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      setError('');
      // request 1: only the current user's submissions, via the submitter tag (case-insensitive so a
      // username's casing doesn't hide results)
      const yoursFilters: Filters = username
        ? { ...filters, tags: { ...(filters.tags ?? {}), [SUBMITTER_TAG]: [username] }, tags_case_insensitive: true }
        : filters;
      ...
      let yoursError = '';
      let allError = '';
      // request 2: everyone's recent submissions, unfiltered by submitter
      const [yoursRes, allRes] = await Promise.all([
        username
          ? listFiles(yoursFilters, (message) => (yoursError = message), true, null)
          : Promise.resolve({ files: [], cursor: null }),
  ...
  const tabs: TabItem<SubmissionsTab>[] = [
    { key: SubmissionsTab.Yours, label: 'Your Submissions', count: yours.length, disabled: !username },
    { key: SubmissionsTab.All, label: 'All Submissions', count: all.length },
  ];
        ...
        <>
          ...
          {active === SubmissionsTab.Yours ? (
            <FileRows
              ...
              onAdd={onAdd}
              onRemove={onRemove}
              emptyLabel="You have no submissions."
              filters={filters}
            />
          ) : (
            <FileRows
