...
/** Whether a result file is currently cached. See {@link ResultFileCache.has}. */
export function hasCachedResultFile(resultId: string, name: string): boolean {
  return sharedCache.has(resultId, name);
}
...
/** Current total bytes retained by the shared cache. */
export function resultFileCacheBytes(): number {
