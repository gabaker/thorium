...
import { Seed, Graph, BranchNode } from '@models/trees';
...
interface AssociationTreeComponentProps {
  initial: Seed; // Thorium tree data object
}

// Browsable Tree component
const AssociationTreeComponent: React.FC<AssociationTreeComponentProps> = ({ initial }) => {
...
export const AssociationTree: React.FC<AssociationTreeProps> = ({ initial }) => {
  return (
    <ErrorBoundary fallback={<RenderErrorAlert page={false} />}>
      <AssociationTreeComponent initial={initial} />
    </ErrorBoundary>
  );
};
