// project imports
import { contextualDisplayEdges, DOWN_DEFAULT_CFG, isStructuralEdge, structuralParentIdsOf, TreeIndex } from './treeHelpers';
...
export function ancestorChain(index: TreeIndex, nodeId: string): string[] {
  ...
  let parent = structuralParentIdsOf(index, nodeId)[0] ?? null;
  while (parent && !visited.has(parent)) {
    visited.add(parent);
    chain.push(parent);
    parent = structuralParentIdsOf(index, parent)[0] ?? null;
  }
  return chain;
}
