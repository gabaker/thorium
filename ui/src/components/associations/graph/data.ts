...
export const processInitialGraphData = (graph: Graph): GraphData => {
  ...
  const addEdge = (source: string, target: BranchNode) => {
    ...
    // DEBUG (remove after diagnosing self-loop crash): a self-loop link (source === target) is a
    // degenerate zero-length link that can crash 3d-force-graph's OrbitControls (reading 'x' of undefined).
    if (sourceNode === targetNode) {
      console.warn('[graph-debug] self-loop link in processInitialGraphData', { node: sourceNode, direction: target.direction });
    }
