...
export const computeBoundingSphere = (nodes: GraphNode[], memberIds: Set<string>): BoundingSphere | null => {
  // average the positioned members into a centroid
  let cx = 0,
    cy = 0,
    cz = 0,
    count = 0;
  for (const n of nodes) {
    if (!memberIds.has(n.id) || n.x === undefined || !isFinite(n.x)) continue;
    cx += n.x;
    cy += n.y ?? 0;
    cz += n.z ?? 0;
    count++;
  ...
  cy /= count;
  cz /= count;
  // radius = farthest positioned member from the centroid, floored for lone nodes
  let radius = LONE_NODE_RADIUS;
  for (const n of nodes) {
    if (!memberIds.has(n.id) || n.x === undefined || !isFinite(n.x)) continue;
