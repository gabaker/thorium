...
// project imports
import { isNode, isEdge } from './utilities';
...
import { DisplayAction, GraphControls } from './GraphControls';
...
const AssociationGraphWrapper: React.FC<AssociationGraphProps> = ({ initial, inView }) => {
  // Global data objects
...
export const AssociationGraph: React.FC<AssociationGraphProps> = ({ initial, inView }) => {
  return (
    <ErrorBoundary fallback={<RenderErrorAlert page={false} />}>
      {inView && <AssociationGraphWrapper inView={inView} initial={initial} />}
    </ErrorBoundary>
  );
};
