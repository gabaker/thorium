...
import { FormattedFileInfoTagKeys, TLPLevels } from '@components/tags/tag_groups';
import { OverlayTipBottom, OverlayTipRight } from '@components/shared/overlay/tips';
import { createReactSelectStyles } from '@utilities/select';
import { deleteTags, getFileDetails, uploadTags } from '@thorpi/files';
import type { Sample } from '@models/files';
import type { TagEntry, Tags, RequestTags } from '@models/tags';
...
const EditableTags = ({ sha256, tags, setDetails, setUpdateError }: EditableTagsProps) => {
  ...
  const [selectedGeneralTags, setSelectedGeneralTags] = useState<ThoriumTagSelectOption[]>([]);
  const [generalTagOptions, setGeneralTagOptions] = useState<ThoriumTagSelectOption[]>([]);
  const [selectedFileInfoTags, setSelectedFileInfoTags] = useState<ThoriumTagSelectOption[]>([]);
  const [fileInfoTagOptions, setFileInfoTagOptions] = useState<ThoriumTagSelectOption[]>([]);
  const [selectedTlpTags, setSelectedTlpTags] = useState<ThoriumTagSelectOption[]>([]);
  const [selectedMBCTags, setSelectedMBCTags] = useState<ThoriumTagSelectOption[]>([]);
  const [selectedAttackTags, setSelectedAttackTags] = useState<ThoriumTagSelectOption[]>([]);
  ...
  const tlpTags: string[] = !(tags && tags.TLP)
    ? []
  ...
  const initialSelectedTLPTags: ThoriumTagSelectOption[] = tlpTags.map((tagValue) => {
    return {
      value: tagValue,
      label: tagValue,
      thoriumTag: { key: 'TLP', value: tagValue },
  ...
  // compare entry keys for sorting
  const compareEntryKeys = (arrayObj1: [string, unknown], arrayObj2: [string, unknown]): number => {
  ...
  const initialGeneralTagOptions: ThoriumTagSelectOption[] = [];
  ...
  const fileInfoTags = Object.fromEntries(Object.entries(tags).filter(([k]) => FormattedFileInfoTagKeys.includes(k.toUpperCase())));
  const initialFileInfoTagOptions: ThoriumTagSelectOption[] = [];
  ...
  const initialMBCTags: ThoriumTagSelectOption[] = [];
  ...
  const mbcTagOptions: ThoriumTagSelectOption[] = String(rawMbcTagDefaults)
    .split('\n')
    .map((mbcValue) => {
      return {
        value: mbcValue,
  ...
  const initialAttackTags: ThoriumTagSelectOption[] = [];
  ...
  const attackTagOptions: ThoriumTagSelectOption[] = String(rawAttackTagDefaults)
    .split('\n')
    .map((attackValue) => {
      return {
        value: attackValue,
  ...
  const commitTagUpdates = async () => {
    ...
    if (deletedTags && Object.keys(deletedTags).length > 0) {
      const data = { tags: deletedTags } as unknown as RequestTags;
    ...
    if (Object.keys(pendingTags).length > 0) {
      const data = { tags: pendingTags } as unknown as RequestTags;
  ...
  const updateUpdatedTagCount = (pending: Record<string, string[]>, deleted: Record<string, string[]>, invalid: string[]) => {
    let count = 0;
  ...
  const updateSelectedTags = (newValue: ActionMeta<ThoriumTagSelectOption>) => {
    ...
    if (newValue.action == 'clear') {
      ...
      const parseRawTag = (rawTag: string): { key: string; value: string | null } => {
        if (rawTag.includes(':')) {
      ...
      if (tag.key && tag.value) {
        ...
        newValue.option['thoriumTag'] = { key: tag.key, value: tag.value };
        if (tag.key in updatedPendingTags) {
          updatedPendingTags[tag.key].push(tag.value);
        } else {
          updatedPendingTags[tag.key] = [tag.value];
          ...
          <Col className="tags-col">
            <Row>
              ...
              <Col className="details-tags-name">
                {editing ? (
                  <Select<ThoriumTagSelectOption, true>
                    isMulti
                    isSearchable
                    isClearable
                    value={selectedTlpTags}
                    styles={tlpTagStyle}
                    onChange={(selected: MultiValue<ThoriumTagSelectOption>, newValue: ActionMeta<ThoriumTagSelectOption>) => {
                  ...
                  tlpTags.map((level) => (
                    <TagBadge resource={Entities.File} key={level} tag={'TLP'} value={level} condensed={false} action={'link'} />
                  ))
                )}
              </Col>
            </Row>
            <Row>
              ...
              <Col className="details-tags-name">
                {editing ? (
                  <CreatableSelect<ThoriumTagSelectOption, true>
                    isMulti
                    isSearchable
                    isClearable
                  ...
                  Object.keys(generalTags)
                    ...
                    .map((tagKey) =>
                      Object.keys(generalTags[tagKey])
                        .sort()
                        .map((tagValue, idx) => (
                          <TagBadge resource={Entities.File} key={idx} tag={tagKey} value={tagValue} condensed={false} action={'link'} />
                        )),
                    )
                )}
              </Col>
            ...
            <Row>
              ...
              <Col className="details-tags-name">
                {editing ? (
                  ...
                  attackTags.map((tagValue) => (
                    <TagBadge resource={Entities.File} key={tagValue} tag={'ATT&CK'} value={tagValue} condensed={false} action={'docs'} />
                  ))
                )}
              </Col>
            </Row>
            <Row>
              ...
              <Col className="details-tags-name">
                {editing ? (
                  ...
                  mbcTags.map((tagValue) => (
                    <TagBadge resource={Entities.File} key={tagValue} tag={'MBC'} value={tagValue} condensed={false} action={'docs'} />
                  ))
                )}
              </Col>
            </Row>
              ...
              <Row>
                <Col>
                  ...
                  {invalidPendingTags.map((tag) => (
                    <TagBadge resource={Entities.File} key={tag} tag={tag} value={''} condensed={true} action={'none'} />
                  ))}
                  <i>
