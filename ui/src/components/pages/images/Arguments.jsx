...
const ArgumentFields = ({ initialArgs, setRequestArguments, errors }) => {
  const parsedArgs = structuredClone(initialArgs);
  if (initialArgs && initialArgs.output && Object.keys(initialArgs.output).includes('Kwarg')) {
    parsedArgs.kwarg = initialArgs.output['Kwarg'];
    parsedArgs.output = 'Kwarg';
  }
  const [args, setArgs] = useState(structuredClone(parsedArgs));
...
const Arguments = ({ args, setRequestArguments, setHasErrors, mode }) => {
