...
const VendorMetaInfo = ({ entity, pendingEntity, handleUpdate, editing }: DetailsMetadataProps<Entities.Vendor>): JSX.Element => {
  ...
  function updatePendingMeta<T extends keyof VendorMetaFields>(field: T, value: VendorMetaFields[T]): void {
    const updates: VendorMetaFields = structuredClone(pendingEntity.metadata.Vendor);
    if (field == 'countries') {
        ...
        <InfoValue>
          {editing ? (
            <SelectInputArray
              ...
              values={
                pendingEntity.metadata.Vendor?.countries
                  ? pendingEntity.metadata.Vendor.countries.map((country: Country) => country.name)
                  : []
              }
        ...
        <InfoValue>
          {editing ? (
            <SelectInputArray
              ...
              values={pendingEntity.metadata.Vendor?.critical_sectors ? pendingEntity.metadata.Vendor.critical_sectors : []}
              onChange={(sectors) => updatePendingMeta('critical_sectors', sectors as CriticalSector[])}
            />
          ) : (
            <>
              <FieldBadge color="DarkRed" noNull={true} field={entity.metadata.Vendor.critical_sectors}></FieldBadge>
            </>
          )}
        </InfoValue>
      </Row>
    ...
    if (data && data.kind == Entities.Vendor) {
      ...
      if (vendor.metadata.Vendor?.critical_sectors) {
        vendor.metadata.Vendor.critical_sectors = vendor.metadata.Vendor.critical_sectors.map(
          (sector: CriticalSector) => sector.replace(/([A-Z])/g, ' $1').trim() as CriticalSector,
        );
      }
      updateEntity(vendor);
    }
...
const VendorDetailsConfig: EntityDetailsConfig<Entities.Vendor> = {
