...
// get compiled functions using filters and an optional cursor
const getCompiledFunctions = async (filters: Filters, cursor: string | null, errorHandler: (error: string) => void) => {
  const listFilters = structuredClone(filters);
  listFilters.kinds = [Entities.CompiledFunction];
  const { entityList, entityCursor } = await listEntities(listFilters, errorHandler, true, cursor);
  return {
    entitiesList: entityList as CompiledFunction[],
...
const CompiledFunctionBrowsingConfig: EntityBrowseConfig<Entities.CompiledFunction> = {
  ...
  fetchEntities: getCompiledFunctions,
};
