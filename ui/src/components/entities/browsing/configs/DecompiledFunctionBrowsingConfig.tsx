...
// get decompiled functions using filters and an optional cursor
const getDecompiledFunctions = async (filters: Filters, cursor: string | null, errorHandler: (error: string) => void) => {
  const listFilters = structuredClone(filters);
  listFilters.kinds = [Entities.DecompiledFunction];
  const { entityList, entityCursor } = await listEntities(listFilters, errorHandler, true, cursor);
  return {
    entitiesList: entityList as DecompiledFunction[],
      ...
      <BrowsingContents>
        ...
        >
          <LinkFields>
            ...
            <EntitySubmitters>{func.submitter ? <small><i>{func.submitter}</i></small> : null}</EntitySubmitters>
          </LinkFields>
        </Link>
...
const DecompiledFunctionBrowsingConfig: EntityBrowseConfig<Entities.DecompiledFunction> = {
  ...
  fetchEntities: getDecompiledFunctions,
};
