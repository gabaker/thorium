...
// get PE sections using filters and an optional cursor
const getPeSections = async (filters: Filters, cursor: string | null, errorHandler: (error: string) => void) => {
  const listFilters = structuredClone(filters);
  listFilters.kinds = [Entities.PeSection];
  const { entityList, entityCursor } = await listEntities(listFilters, errorHandler, true, cursor);
  return {
    entitiesList: entityList as PeSection[],
      ...
      <BrowsingContents>
        ...
        >
          <LinkFields>
            ...
            <EntitySubmitters>{section.submitter ? <small><i>{section.submitter}</i></small> : null}</EntitySubmitters>
          </LinkFields>
        </Link>
