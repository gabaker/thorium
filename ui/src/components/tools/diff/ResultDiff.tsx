...
import { diffForFile, diffForValue, resultToYaml } from './resultDiff';
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [b, c] = await Promise.all([
        baseResult?.files?.includes(target) ? getResultsFile(sha256, tool, baseResult.id, target, errorHandler) : Promise.resolve(null),
        compareResult?.files?.includes(target)
          ? getResultsFile(sha256, tool, compareResult.id, target, errorHandler)
