...
const TabButton = styled.button<{ $active: boolean }>`
  ...
  color: ${({ $active }) => ($active ? 'var(--thorium-highlight-text)' : 'var(--thorium-secondary-text)')};
...
const CountBadge = styled.span<{ $active: boolean }>`
  ...
  font-weight: 600;
  line-height: 1;
  background: ${({ $active }) => ($active ? 'var(--thorium-highlight-text)' : 'var(--thorium-highlight-panel-bg)')};
