...
import { OceanJsonTheme } from './jsonTheme';
...
const JsonRenderer: React.FC<FileRendererProps> = ({ input }) => {
  const parsed = useMemo<{ ok: boolean; value: unknown }>(() => {
    const text = input.text ?? decodeText(input.bytes);
    try {
  ...
  return (
    <TreeWrapper>
      <JSONTree data={parsed.value} shouldExpandNodeInitially={() => true} hideRoot={true} theme={OceanJsonTheme} invertTheme={false} />
    </TreeWrapper>
  );
};
