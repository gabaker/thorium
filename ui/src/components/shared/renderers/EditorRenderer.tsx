...
// project imports
import { decodeText, formatFromFileName } from './detect';
import { prettify, prettifyFormatFor } from './prettify';
...
const EditorRenderer: React.FC<FileRendererProps> = ({ input, value, onTextChange, formatHint }) => {
  ...
  const seed = useMemo(
    () => prettify(input.text ?? decodeText(input.bytes), prettifyFormatFor(input.fileName ?? '', input.bytes)),
    [input.text, input.bytes, input.fileName],
  );
  const [internal, setInternal] = useState(seed);

  // report the initial buffer upward once (mount) so a consumer's toolbar can act on it before any
  // edit — only relevant in the uncontrolled fallback; controlled callers already hold the seed
  useEffect(() => {
    if (!controlled) onTextChange?.(internal);
  }, []);
