        ...
        Object.keys(tagGrouping)
          ...
          .map((tagKey) =>
            Object.keys(tagGrouping[tagKey])
              .sort()
              .map((tagValue) => (
                <TagBadge key={`${tagKey}_${tagValue}`} tag={tagKey} value={tagValue} condensed={true} action={action} resource={resource} />
              )),
          ),
      )}
    </>
...
export function resolveFieldLayout(field: InfoField): FieldLayout {
  ...
  if (field.render === FieldRender.Markdown || field.render === FieldRender.Text || field.render === FieldRender.Path) {
