...
import { buildTagBrowseHref } from '@components/tags/utilities';
...
// per-kind detail rows; `treeNodeToInfo` assembles these into sections.
function entityFields(entity: NonNullable<TreeNode['Entity']>): { identifiers: InfoField[]; fields: InfoField[] } {
  ...
  switch (entity.kind) {
    case Entities.Device: {
      ...
      return {
        ...
        fields: compact([
          ...
          { label: 'Vendors', links: d.vendors?.map((v) => ({ text: v.name, href: entityHref(Entities.Vendor, v.id) })), layout: FieldLayout.Inline },
    ...
    case Entities.WindowsProcess: {
      ...
      return {
        identifiers: [],
        // `name` is intentionally omitted — it duplicates the entity name shown as the title
        fields: compact([
          { label: 'PID', value: p.pid },
          { label: 'Parent PID', value: p.parent_pid },
          { label: 'Image Path', value: p.image_path, render: FieldRender.Path, layout: FieldLayout.Inline },
          { label: 'Command', value: p.command, render: FieldRender.Path, layout: FieldLayout.Inline },
    ...
    case Entities.SigmaRule: {
      const s = m.SigmaRule as { score?: number; applies_to?: string[]; actions?: unknown[] };
      return {
        identifiers: [],
        fields: compact([
    ...
    case Entities.CompiledFunction: {
      const f = m.CompiledFunction as { address?: number; disassembly?: unknown[] };
      return {
        identifiers: [],
        fields: compact([
    ...
    case Entities.DecompiledFunction: {
      const f = m.DecompiledFunction as { address?: number; tools?: string[] };
      return {
        identifiers: [],
        fields: compact([
          { label: 'Address', value: f.address != null ? formatAddress(f.address) : undefined, layout: FieldLayout.Inline },
