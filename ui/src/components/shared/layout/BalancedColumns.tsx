...
/// overflowing, and `align-items: start` so each column flows to its own height. `$hidden` keeps the
/// tiles laid out (so they still measure) but invisible until the first real-height balance commits,
/// hiding the one-time round-robin→balanced reshuffle on first paint.
...
export const BalancedColumns: React.FC<BalancedColumnsProps> = ({
  ...
  }, [columns, columnWidth, anchorCount, balanceStrategy, layoutKey]);
