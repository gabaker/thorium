...
///
/// Missing real variant comments until https://github.com/kube-rs/kube/issues/1821 is resolved
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema)]
pub enum LdapIdCast {
    // Read it as an int
    Int(usize),
...
pub const fn default_csrf_expire() -> u64 {
    600
}

/// Helps serde default the oauth account link lifetime to 1 day
pub const fn default_oauth_link_expire() -> u64 {
    86_400
}
...
fn default_search_streamer_max_document_size() -> ByteSize {
    ByteSize::mib(100)
}

/// Returns the default maximum request size for the search-streamer
fn default_search_streamer_max_request_size() -> ByteSize {
    ByteSize::mib(100)
}

/// Build the ischema generator for ByteSize
fn bytesize_schema_gen(_: &mut SchemaGenerator) -> Schema {
...
pub struct SearchStreamer {
    ...
    #[schemars(example = "100 MiB")]
    #[schemars(schema_with = "bytesize_schema_gen")]
    pub max_document_size: ByteSize,
...
///
/// Missing vairant comments until https://github.com/kube-rs/kube/issues/1821 is resolved
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema)]
