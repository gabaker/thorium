...
#[cfg(feature = "client")]
pub use client::{CountCursor, CountCursorSupport, Cursor, DateOpts};

#[cfg(feature = "sync")]
pub use client::{CountCursorBlocking, CursorBlocking};
...
mod client {
    ...
    impl DateOpts {
        ...
        pub fn year(year: u64, end: bool) -> Result<DateTime<Utc>, Error> {
            ...
            let raw = if end {
                format!("{year}-00-01T00:00:00-00:00")
            } else {
                format!("{year}-12-31T23:59:59-00:00")
            };
    ...
    /// The data that the cursor from the API alone will return
    #[cfg_attr(feature = "sync", thorium_derive::blocking_struct(getters("data")))]
    pub struct Cursor<T>
    where
        for<'de> T: Deserialize<'de>,
    {
    ...
    pub trait CountCursorSupport: Sized {
        ...
        fn consume(self) -> Result<(Option<Uuid>, usize, Self::Data), Error>;
    }

    #[cfg_attr(feature = "sync", thorium_derive::blocking_struct(getters("data")))]
    pub struct CountCursor<T: CountCursorSupport> {
        /// The ID for this cursor if more data can be pulled
        pub id: Option<Uuid>,
        /// A single page worth of data
        pub data: T::Data,
    ...
    #[cfg_attr(feature = "sync", thorium_derive::blocking_struct)]
    impl<T: Serialize + CountCursorSupport> CountCursor<T>
    where
        for<'de> T: Deserialize<'de>,
    {
        /// Create a new cursor object
            ...
            Q: Serialize + ?Sized,
        {
            ...
            let req = client
                ...
                .query(query);
            // send request and build the data and id for this cursor
            let raw = send_build!(client, req, T)?;
            // extract the needed data from our cursor
            let (cursor, gathered, data) = raw.consume()?;
            // build our final cursor object
        ...
        pub async fn refill(&mut self) -> Result<(), Error> {
            ...
            let raw = loop {
                ...
                let req = self
                    ...
                    .query(&query);
                // send request and build a raw cursor
                match send_build!(self.client, req, T) {
                    Ok(raw) => break raw,
                    Err(error) => {
                        // if retry is enabled then check if we should retry or just fail
                        if self.retry {
