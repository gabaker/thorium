...
async fn get_image(
    ...
    match &target.image {
        Some(image_path) => {
            // download the icon from S3 with its metadata
            let get_object = target.download_graphic(image_path, &state.shared).await?;
...
pub fn mount(router: Router<AppState>) -> Router<AppState> {
    router
        ...
        .route("/users/image", post(upload_image).delete(delete_image))
