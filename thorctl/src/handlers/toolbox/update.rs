...
use thorium::models::{
    BurstableResourcesUpdate, EventTrigger, Image, ImageBan, ImageBanUpdate, ImageRequest,
    ImageUpdate, Pipeline, PipelineBanUpdate, PipelineRequest, PipelineUpdate, Resources,
...
pub fn calculate_image_update_from_mergeable(
    ...
    let bans = match resolved.bans {
        Some(new_bans) => calculate_bans_update(image.bans, new_bans)?,
        None => ImageBanUpdate::default(),
    };
    Ok(Some(ImageUpdate {
        external: None,
...
fn calculate_bans_update(
    ...
    if old_bans == new_bans {
        return Ok(ImageBanUpdate::default());
    }
...
pub fn calculate_pipeline_update_from_mergeable(
    ...
    let resolved_yaml = serde_yaml::to_string(&resolved)
        .map_err(|e| Error::new(format!("Failed to serialize resolved pipeline to YAML: {e}")))?;
    if current_yaml == resolved_yaml {
        return Ok(None);
    }
    ...
    let order = if pipeline.order == resolved.order {
        ...
        Some(serde_json::to_value(&resolved.order)
